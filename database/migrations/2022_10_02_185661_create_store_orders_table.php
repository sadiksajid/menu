...
class CreateStoreOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('store_orders', function (Blueprint $table) {
            ...
            $table->enum('status', ['pending', 'declined', 'confirmed', 'shipped', 'delivered', 'ready', 'canceled', 'dispute', 'return'])->default('pending');
            $table->integer('shipping_cost')->default(0);
            $table->boolean('from_web')->default(0);
            $table->enum('order_type', ['shipping', 'coming'])->default('shipping');
