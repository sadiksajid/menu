...
$(document).on("click", "#s_CodpostalListO li", function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            Livewire.emit('ssaveCodeResponse', response[0].ville_id, response[0].ville, response[0].id, response[0].quartier, code);
        }
    });
});
...
$(document).on("click", "#s_CityListO li", function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            Livewire.emit('ssaveVilleResponse', response[0].ville_id, response[0].code_postal, response[0].id, response[0].quartier, code);
        }
    });
});
...
$(document).on("click", "#QuartierList li", function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            document.getElementById('ville_id').value = response[0].ville_id;
            $('#vilt').hide();
            Livewire.emit('saveQuartierResponse', response[0].ville_id, response[0].ville, response[0].id, response[0].code_postal, code);
...
$(document).ready(function() {
    ...
    $(document).on("click", "#reg_QuartierList li", function() {
        ...
        console.log($('#reg_quartier_id').val());
        var code = iid; //to get content of "value" attrib
        $.ajax({
            url: "/getquartierinfo",
            data: { code: code },
...
function showRegMap() {
    var x = document.getElementById("bigmaprec");
    if (x.style.display === "none") {
        x.style.display = "block";
    } else {
        x.style.display = "none";
